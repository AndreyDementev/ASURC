...
void WeightSaveAndDisplay(){
  ...
  SdEjectButtonStatus = SdEjectButton;                    //запоминаем состояние кнопки вынуть SD
  SdEjectButton = true;                                   //чтобы не писать на внешний SD устанавливаем ее нажатой
#endif
  sResult = SaveDataToSD(StartWeight, EndWeight, DiffWeight) + "\n";
  
#ifdef _NIGHT_SAVE_SD_
  SdEjectButton = SdEjectButtonStatus;                    //восстанавливаем состояние кнопки
#endif
...
#ifdef ARDUINO_AVR_MEGA2560
#elif ARDUINO_SAM_DUE
  watchdogEnable(WD_TIME_OUT);                                   //запуск watchdog
#endif
}
...
#ifdef _NIGHT_SAVE_SD_
      SaveDataToSD(0, 0, 0);                          //сначала синхронизируем внешний SD
#endif

      DisplayMessage("SD карточку можно извлечь.\nНе отпускать кнопку без карточки!", MSG_WARNING);
      SdEjectButton = !SdEjectButton;                 //затем устанавливаем статус нажатой кнопки
    }
    if(SdEjectButton && digitalRead(SD_EJECT_BUTTON_PIN)){
...
//----------------------------------- Цикл отображение веса ----------------
  ...
  if (int(millis()/500) - TimeS) { //период 
    ...
    if ((t.hour >= 3) && (t.hour < 4)) Rebooted = false;  //готовимся, даем команду на перезагрузку ночью
    Serial.print(t.hour); Serial.print("     "); Serial.println(Rebooted);
    
    if ((t.hour >= 4) && (t.hour < 5)){                   //если глубокая ночь
      if (!Rebooted){                                   //если еще не перезагружена
//        if (rtc.getTimeAfterLastRead() > 3600){       //и час никто не работает
